...
#    targets[:,0:6] = 751
    optargets = Variable(torch.FloatTensor(range(24)).cuda())
    optargets = optargets.unsqueeze(0).expand(targets.size(0),optargets.size(0))
    Rtargets = Variable(torch.FloatTensor(range(1,num_parts+1)).cuda())*24/num_parts   #generating the ending_index of each pre-deifned region
    ...
    prange = (Rtargets-24*start_ratio.float().cuda())/ratio.float().cuda()   #64*6                                     #generating the ending_index of each region after crop and resizing operation
    ...
    for i in range(0,num_parts):
        tmp.append((optargets.lt(prange[:,:,i])))                                      #return 1 if a pixel is within region i or i-1, ..,0
